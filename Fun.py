...
def find_colour_of_image(image):
    if type(image) == str:
        image = pg.image.load(os.path.join(image)).convert()
    ...
    for y in range(height):
        for x in range(width):
            if image.get_at((x, y)) not in colours:
                colours.append(image.get_at((x, y)))
    print(colours)
...
def status_manager(self, entities):
    ...
    if self.status["Burning"] > 0:
        entities["particles"].append(FireParticle([self.pos[0], self.pos[1]]))
...
def cutscene_mode(self, entities, bullets):
    ...
    if self.is_player:
        ...
        if CUTSCENE_SKIP_TIMER[0] > 0:
            entities["UI particles"].append(CutsceneSkip(CUTSCENE_SKIP_TIMER[0]))
    ...
    if self.cutscene_mode[0]["stage"] <= entities["cutscene stage"]:
        for task in self.cutscene_mode[0]["task"]:
            ...
            if task[0] == "Goto":
                ...
                entities["particles"].append(
                    FloatingTextType2([self.pos[0], self.pos[1] - 16], 18, task[1], task[2], 1))
            elif task[0] == "Noise":
                if task[-1]:
                ...
                if task[-1]:
                    entities["UI particles"].append(MissionTitleCard(task[1][0], task[1][1]))
                    task[-1] = False
